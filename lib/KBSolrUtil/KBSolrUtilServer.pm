package KBSolrUtil::KBSolrUtilServer;
...
sub call_method {
    ...
    my $ctx = KBSolrUtil::KBSolrUtilServerContext->new($self->{loggers}->{userlog},
...
{
    # Service KBSolrUtil requires authentication.
    ...
    my @result;
    {
        ...
        my $stderr = KBSolrUtil::KBSolrUtilServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service KBSolrUtil.");
    }
...
package KBSolrUtil::KBSolrUtilServerContext;

use strict;

=head1 NAME

KBSolrUtil::KBSolrUtilServerContext
...
package KBSolrUtil::KBSolrUtilServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = KBSolrUtil::KBSolrUtilServer->new(
