...
sub new
{
    ...
    my $self = {
	client => Workspace::nullClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for Workspace::nullClient\n";
    }
    if ($sMajor == 0) {
        warn "Workspace::nullClient version is $svr_version. API subject to change.\n";
    }
}
...
package Workspace::nullClient::RpcClient;
